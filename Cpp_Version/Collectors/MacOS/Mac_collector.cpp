...
void NVMLogCollectorMac::setKDFDebugFlag() {
    ...
    try {
        ...
        cerr << "[!] Invalid hexadecimal input. Please enter a valid hex value." << endl;
    }
}
...
void NVMLogCollectorMac::collectKDFLogs() {
    ...
    std::string desktopPath = std::string(homeDir) + "/Desktop/kdf_logs.log";
    
    // Construct the log collection command
    std::string cmd = "sudo log stream --predicate 'process == \"com.cisco.anyconnect.macos.acsockext\"' "
                     "--style syslog > " + desktopPath + " &";
    
    logger->info("Executing log collection command...");
    int result = system(cmd.c_str());
    logger->info("KDF log collection started. Logs will be saved to: " + desktopPath);
    if (result == 0) {
        logger->info("KDF Logs Collected Succesfully");
    } else {
        logger->error("Failed to start KDF log collection. Error code: " + std::to_string(result));
    }
}
...
void NVMLogCollectorMac::organizeAndArchiveLogs() {
    ...
    // 2. Move all log files to nvm_logs directory
    ...
    system(moveCmd.c_str()); // Ignore errors as some files might not exist
